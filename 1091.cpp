...
{
    return node.l < l && node.l >= 0 && node.m >= 0 && node.m < m && node.n < n && node.n >= 0;
}
...
{
    cin >> m >> n >> l >> bonder;
    visited = vector<vector<vector<bool>>>(l, vector<vector<bool>>(m, vector<bool>(n, false)));
    mat = vector<vector<vector<int>>>(l, vector<vector<int>>(m, vector<int>(n, 0)));
    for (int i = 0; i < l; ++i)
    {
        for (int j = 0; j < m; ++j)
        {
